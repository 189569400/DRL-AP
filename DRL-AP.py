...
def startTemCode():
    ...
    if os.WEXITSTATUS(status) != 0:
        print("DRL-AP: Random topology generation failed => abort")
        return
...
def start_function(model):
    ...
    if model == 'logical_attack':
        print("DRL-AP: Operation mode:  {}".format("Attack on logical network"))
        print("DRL-AP: Target topology: {}".format(TOPOLOGY_PATH + LOGICAL_ATTACK_TOPOLOGY))
        startTrainCode('logical_attack')
    elif model == 'logical_attack_gen':
        print("DRL-AP: Operation mode:  {}".format("Attack on generated logical network"))
        print("DRL-AP: Target topology: {}".format(TOPOLOGY_PATH + GENERATION_TOPOLOGY_FINAL))
        startTemCode()
    elif model == 'real_attack':
        print("DRL-AP: Operation mode:  {}".format("Attack on real network"))
        print("DRL-AP: Target topology: {}".format(TOPOLOGY_PATH + REAL_ATTACK_TOPOLOGY))
        startRealAttackCode()
    else:
        print("DRL-AP: ERROR: Unrecognized operation mode: {}".format(model))
...
if __name__ == '__main__':
    try:
        ...
        print ("\nDRL-AP: Keyboard interrupt detected => end execution")
        sys.exit(1)
